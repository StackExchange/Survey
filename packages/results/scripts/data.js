...
// --- schema.org ----------------------------------------------------------------
//
// Each page's JSON-LD `@graph`, built here rather than assembled by hand in six
// routes. Every node is a pure function of the payload, and every page is
// prerendered, so there is nothing a route knows at render time that this
// doesn't. A question is a `PropertyValue` in `Dataset.variableMeasured`,
// deliberately not `Question`/`suggestedAnswer`: that vocabulary is community
// Q&A and would assert respondents proposed the options.
...
const ids = {
	...
	dataset: (p) => `${siteUrl}${p}#dataset`,
	page: (p) => `${siteUrl}${p}#webpage`,
}
...
const breadcrumbs = (trail) => ({
	...
	itemListElement: trail.map((item, i) => ({ '@type': 'ListItem', position: i + 1, name: item.name, item: `${siteUrl}${item.path}` })),
})

const webPage = (page, mainEntity) => ({
	'@type': 'WebPage',
...
// path on this site.
const download = (format, name, url) => ({
	'@type': 'DataDownload',
...
function seoFor({ survey, years, chapters, chapterPayloads }) {
	...
	const yearGraph = [
		...
		breadcrumbs([crumb, { name: year, path: `/${year}` }]),
		{
			...
			keywords: KEYWORDS,
			hasPart: chapters.map(({ id }) => ({ '@id': ids.dataset(`/${year}/${id}/data`) })),
	...
	for (const c of chapters) {
		...
		const figures = (chapterPayloads[c.id]?.sections ?? []).flatMap((s) => s.questions)
		...
		const chapterDataset = {
			...
			variableMeasured: figures.map((q) => ({
				'@type': 'PropertyValue',
				propertyID: q.dataId,
			...
			question[`${c.id}/${q.id}`] = [
				...
				]),
				{
					...
					variableMeasured: group.data
						.filter((row) => row.response != null && typeof row.pct === 'number')
						.map((row) => ({
							'@type': 'PropertyValue',
							propertyID: q.dataId,
							name: row.response,
...
export async function generate() {
	...
	const seo = seoFor({ survey, years, chapters: summaries, chapterPayloads })
